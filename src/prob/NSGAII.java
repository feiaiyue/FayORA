...
//    result
    ...
    public List<Individual> run(int timeLimit) {
...
//                test:
                System.out.println("parent1Size" + parent1.cars.size());
...
//        Collections.shuffle(cars, new Random(seed));
...
//    }
    public List<Individual> greedySortInitializePopulation() {
        ...
        for (Individual individual : population) {
            ...
            long seed = System.currentTimeMillis();
            Collections.shuffle(carsANotSameColorCopy, new Random(seed));
            Collections.shuffle(carsBNotSameColorCopy, new Random(seed));
            Collections.shuffle(splitCarsASameColorCopy, new Random(seed));
            Collections.shuffle(splitCarsBSameColorCopy, new Random(seed));
            ...
            Collections.shuffle(combinedCars, new Random(seed1));
...
//        test
        ...
        if (temp.cars.size() == 0) {
            for(Individual individual : candidates) {
                System.out.println(individual.cars.get(index).carNo);
            }
        }
        return temp;
...
//                .orElseThrow();
    ...
    private void crossover(Individual parent1, Individual parent2, Individual offspring1, Individual offspring2) {
        ...
        if (rand.nextDouble() < crossoverProbability) {
            int crossoverPoint = rand.nextInt(parent1.cars.size());
            for (int i = 0; i < crossoverPoint; i++) {
                offspring1.cars.add(parent1.cars.get(i));
                offspring2.cars.add(parent1.cars.get(i));
